...
describe("User Status - Core Functions", () => {
	...
	it("should get user status", async () => {
		const { setUserStatus, getUserStatus } = await import(
			"../lib/appwrite-status"
		);
	...
	it("should get multiple users statuses", async () => {
		const { setUserStatus, getUsersStatuses } = await import(
			"../lib/appwrite-status"
		);
	...
	it("should update lastSeen timestamp", async () => {
		const { setUserStatus, updateLastSeen, getUserStatus } = await import(
			"../lib/appwrite-status"
		);
	...
	it("should set offline status", async () => {
		const { setUserStatus, setOffline, getUserStatus } = await import(
			"../lib/appwrite-status"
		);
...
describe("User Status - Batch Operations", () => {
	it("should handle batch status retrieval efficiently", async () => {
		...
		await Promise.all(
			userIds.map((id) => setUserStatus(id, "online", `Status for ${id}`))
		);
