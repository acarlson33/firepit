...
vi.mock("../lib/appwrite-profiles", () => {
	...
	return {
		getProfilesByUserIds: async (userIds: string[]) => {
		...
		getAvatarUrl: (fileId: string) =>
			`http://localhost/storage/buckets/avatars/files/${fileId}/view?project=test-project`,
	};
});
...
describe("Message Enrichment", () => {
	describe("Batch Enrichment", () => {
		...
		it("should enrich multiple messages with profile data", async () => {
			const { enrichMessagesWithProfiles } = await import(
				"../lib/enrich-messages"
			);
	...
	describe("Single Message Enrichment", () => {
		it("should enrich a single message", async () => {
			const { enrichMessageWithProfile } = await import(
				"../lib/enrich-messages"
			);
