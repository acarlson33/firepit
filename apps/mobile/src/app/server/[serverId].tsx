...
import { createChannelMessage, fetchChannels } from "@/lib/firepit";
...
export default function ServerWorkspaceScreen() {
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const response = await createChannelMessage(instanceUrl, accessToken, {
          ...
          <ThemedView style={styles.shell}>
            ...
            >
              ...
              <ThemedText type="subtitle">
                {normalizedServerId ?? "Unknown server"}
              </ThemedText>
              <ThemedText themeColor="mutedForeground" style={styles.copy}>
                This slice brings in the server channel list and keeps the next
                message view one tap away.
              </ThemedText>
              <View style={styles.pillRow}>
                <StatusPill
                  label={signedIn ? "signed in" : "sign in required"}
                ...
                />
                {selectedChannel ? (
                  <StatusPill label="channel selected" tone="success" />
                ) : null}
              </View>
            </ThemedView>
            ...
            >
              ...
              <ThemedText themeColor="mutedForeground" style={styles.copy}>
                Pick a channel to continue into the message timeline.
              </ThemedText>
                ...
                <View style={styles.list}>
                  {channels.map((channel) => (
                    <ChannelCard
                      ...
                      channel={channel}
                      selected={channel.$id === normalizedChannelId}
            ...
            >
              <View style={styles.sectionHeaderRow}>
                <ThemedText type="smallBold">Selected channel</ThemedText>
                {selectedChannel ? (
                  <ActionButton
                    label="Clear"
                    tone="ghost"
              ...
              {selectedChannel ? (
                <View style={styles.selectedPanel}>
                  ...
                  <View style={styles.pillRow}>
                    <StatusPill
                      label={selectedChannel.type ?? "text"}
                      tone={
                        selectedChannel.type === "announcement"
                          ? "warning"
                          : selectedChannel.type === "voice"
                  ...
                  {selectedChannel.topic ? (
                    ...
                    <ThemedText
                      ...
                      style={styles.copy}
                    >
                      No topic is set for this channel yet.
                    </ThemedText>
                  )}
                  <ThemedText themeColor="mutedForeground" style={styles.copy}>
                    The next slice will replace this shell with the message
                    timeline and thread navigation.
                  </ThemedText>
                </View>
              ) : (
                <ThemedText themeColor="mutedForeground">
                  Select a channel above to open the next workspace view.
                </ThemedText>
              )}
            </ThemedView>
            ...
            >
              <View style={styles.sectionHeaderRow}>
                <ThemedText type="smallBold">Message timeline</ThemedText>
                {selectedChannel ? (
                  <StatusPill
                    label={selectedChannel.name ?? "channel"}
              ...
              {selectedChannel ? (
                <View style={styles.timelineBlock}>
                  <ThemedText themeColor="mutedForeground" style={styles.copy}>
                    Historical messages are still wired to the server contract.
                    This slice keeps newly sent messages visible immediately.
                  </ThemedText>
                  ...
                  <View style={styles.composerCard}>
                    ...
                    <ChatInput
                      ...
                      disabled={messageSendState === "loading"}
                      onMentionsChange={() => {}}
                    ...
                    <View style={styles.composerActions}>
                      <ActionButton
                        label={
                          messageSendState === "loading" ? "Sending…" : "Send"
                        }
                        onPress={sendMessage}
                        tone="primary"
                      />
                  ...
                  {timelineMessages.length > 0 ? (
                    ...
                    <ThemedView type="secondary" style={styles.emptyTimeline}>
                      ...
                      <ThemedText
                        ...
                        style={styles.copy}
                      >
                        Send the first message to seed the timeline while the
                        remote list endpoint is added.
                      </ThemedText>
                    </ThemedView>
                  )}
                </View>
                ...
                <ThemedText themeColor="mutedForeground">
                  Select a channel to open its message timeline.
                </ThemedText>
              )}
            </ThemedView>
            ...
            >
              ...
              <ThemedText themeColor="mutedForeground" style={styles.copy}>
                Use the browser to jump between servers while the timeline slice
                is added.
              </ThemedText>
              <ThemedView type="secondary" style={styles.backLink}>
                <ThemedText
                  type="smallBold"
...
function MessageCard({
  ...
  onToggleReaction?: (emoji: string, isAdding: boolean) => Promise<void> | void;
}) {
  const theme = useTheme();
  const imageUrl =
  ...
  return (
    ...
    >
      <View style={styles.messageHeader}>
        ...
        {message.local ? <StatusPill label="local" tone="warning" /> : null}
      </View>
      {children ? (
        <>{children}</>
      ) : (
        <ThemedText style={styles.messageBody}>{message.text ?? ""}</ThemedText>
      )}
        ...
        <View style={{ flexDirection: "row", marginTop: Spacing.one }}>
          {reactions.map((r) => (
            <ReactionButton
              ...
              reaction={r}
              onToggle={(emoji, adding) => onToggleReaction?.(emoji, adding)}
            />
          ))}
        </View>
      ) : null}
      <View style={styles.messageFooter}>
        ...
        {message.replyToId ? <StatusPill label="reply" tone="neutral" /> : null}
      </View>
    </ThemedView>
  );
}
    ...
    >
      ...
      <View style={styles.channelMetaRow}>
        ...
        {channel.memberCount != null ? (
          <StatusPill label={`${channel.memberCount} members`} tone="neutral" />
        ) : null}
      </View>
    </Pressable>
  );
