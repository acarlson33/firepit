    ...
    describe("POST /api/typing", () => {
        it("should require authentication", async () => {
            ...
            const request = new NextRequest(
                "http://localhost:3000/api/typing",
                {
                    method: "POST",
        ...
        it("should return 403 when user cannot type in a channel", async () => {
            const { getServerSession } = await import("@/lib/auth-server");
            const { getChannelAccessForUser } =
                await import("@/lib/server-channel-access");
            ...
            vi.mocked(getChannelAccessForUser).mockResolvedValue({
                serverId: "server-1",
                isServerOwner: false,
                isMember: true,
                canRead: true,
        ...
        it("should create typing status when document doesn't exist", async () => {
            ...
            const mockCreate = vi
                .fn()
                .mockResolvedValue({ $id: "typing-doc-123" });
            const mockUpdate = vi
                .fn()
